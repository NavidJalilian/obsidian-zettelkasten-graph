...
// node-manipulator.ts
var NodeManipulator = class {
  constructor(vault, parser, svg) {
    this.commandHistory = [];
    this.historyIndex = -1;
   ...
   */
  createEnhancedDragBehavior(nodes, graph, onGraphUpdate) {
   ...
   */
  startDragOperation(node, event) {
    select_default2(event.sourceEvent.target.closest(".node")).classed("dragging", true);
    this.dropZoneIndicator = this.svg.select("g").append("circle").attr("class", "drop-zone-indicator").attr("r", 0).style("opacity", 0);
    this.relationshipPreview = this.svg.select("g").append("line").attr("class", "relationship-preview").style("opacity", 0);
  }
  /**
   * Update drag position and visual state
   */
  updateDragPosition(node, event) {
    node.fx = event.x;
    node.fy = event.y;
  }
  /**
   ...
   */
  detectDropZone(draggedNode, allNodes, event) {
    ...
    for (const targetNode of allNodes) {
      ...
      const distance = Math.sqrt(
        Math.pow(mouseX - targetNode.x, 2) + Math.pow(mouseY - targetNode.y, 2)
      );
   ...
   */
  calculateDropZone(draggedNode, targetNode) {
   ...
   */
  validateHierarchicalMove(draggedNumber, targetNumber) {
   ...
   */
  calculateNewNumber(draggedNumber, targetNumber, position) {
    switch (position) {
      case "child":
   ...
   */
  updateVisualFeedback(dropZone) {
    if (!this.dropZoneIndicator || !this.relationshipPreview)
      return;
    if (dropZone) {
      const targetNode = dropZone.targetNode;
      this.dropZoneIndicator.attr("cx", targetNode.x).attr("cy", targetNode.y).attr("r", 35).classed("invalid", !dropZone.isValid).style("opacity", 1);
      selectAll_default2(".node").classed("drag-target", false).classed("invalid-drop-target", false);
      select_default2(`.node[data-id="${targetNode.id}"]`).classed("drag-target", dropZone.isValid).classed("invalid-drop-target", !dropZone.isValid);
    } else {
   ...
   */
  async executeNodeMove(draggedNode, dropZone, graph, onGraphUpdate) {
    try {
      const command = {
        type: "move",
   ...
   */
  async performNodeMove(draggedNode, dropZone, graph) {
   ...
   */
  updateParentChildRelationships(draggedNode, dropZone, graph) {
    if (draggedNode.zettel.parentId) {
      const oldParent = graph.nodes.get(draggedNode.zettel.parentId);
      if (oldParent) {
        oldParent.children = oldParent.children.filter((id2) => id2 !== draggedNode.id);
      }
    }
   ...
   */
  async renumberChildNodes(oldParentNumber, newParentNumber, graph) {
   ...
   */
  async updateNodeFile(node, oldNumber, newNumber) {
  ...
  endDragOperation() {
    ...
    selectAll_default2(".node").classed("dragging", false).classed("drag-target", false).classed("invalid-drop-target", false);
  }
  /**
   * Show feedback message to user
   */
  showFeedback(message, type2 = "info") {
    if (this.feedbackOverlay) {
      this.feedbackOverlay.remove();
    }
    this.feedbackOverlay = document.createElement("div");
    this.feedbackOverlay.className = `manipulation-feedback ${type2}`;
   ...
   * Undo last operation
   */
  async undo(graph, onGraphUpdate) {
   ...
   */
  async redo(graph, onGraphUpdate) {
   ...
   */
  async reverseCommand(command, graph) {
   ...
   */
  async executeCommand(command, graph) {
    const node = graph.nodes.get(command.nodeId);
    if (!node)
      return;
...
var AnimationSystem = class {
   ...
   */
  animateNodeMovement(nodeSelection, targetPosition, config = {}) {
    var _a;
    const finalConfig = { ...this.defaultConfig, ...config };
    const nodeId = ((_a = nodeSelection.datum()) == null ? void 0 : _a.id) || Math.random().toString();
    return new Promise((resolve) => {
      this.cancelAnimation(nodeId);
      const transition2 = nodeSelection.transition().duration(finalConfig.duration).delay(finalConfig.delay || 0).ease(cubicInOut);
      this.activeAnimations.set(nodeId, transition2);
      transition2.attr("transform", `translate(${targetPosition.x}, ${targetPosition.y})`).on("end", () => {
   ...
   */
  animateNodeScale(nodeSelection, scale, config = {}) {
    var _a;
    const finalConfig = { ...this.defaultConfig, ...config };
    const nodeId = `scale-${((_a = nodeSelection.datum()) == null ? void 0 : _a.id) || Math.random()}`;
    return new Promise((resolve) => {
      const transition2 = nodeSelection.select("circle").transition().duration(finalConfig.duration).delay(finalConfig.delay || 0).ease(cubicInOut);
      this.activeAnimations.set(nodeId, transition2);
      transition2.attr("transform", `scale(${scale})`).on("end", () => {
        this.activeAnimations.delete(nodeId);
        resolve();
      });
    });
   ...
   */
  animateNodeColor(nodeSelection, targetColor, config = {}) {
    var _a;
    const finalConfig = { ...this.defaultConfig, ...config };
    const nodeId = `color-${((_a = nodeSelection.datum()) == null ? void 0 : _a.id) || Math.random()}`;
    return new Promise((resolve) => {
      const transition2 = nodeSelection.select("circle").transition().duration(finalConfig.duration).delay(finalConfig.delay || 0).ease(cubicInOut);
      this.activeAnimations.set(nodeId, transition2);
      transition2.attr("fill", targetColor).on("end", () => {
        this.activeAnimations.delete(nodeId);
        resolve();
      });
    });
   ...
   */
  animateTextChange(textSelection, newText, config = {}) {
    const finalConfig = { ...this.defaultConfig, ...config };
    const textId = `text-${Math.random()}`;
    return new Promise((resolve) => {
      const fadeOut = textSelection.transition().duration(finalConfig.duration / 2).ease(cubicInOut);
      this.activeAnimations.set(textId, fadeOut);
      fadeOut.attr("transform", "scale(0.8)").style("opacity", 0).on("end", () => {
        textSelection.text(newText);
        const fadeIn = textSelection.transition().duration(finalConfig.duration / 2).ease(cubicInOut);
        fadeIn.attr("transform", "scale(1)").style("opacity", 1).on("end", () => {
   ...
   */
  animateLinkChange(linkSelection, startPos, endPos, config = {}) {
    const finalConfig = { ...this.defaultConfig, ...config };
    const linkId = `link-${Math.random()}`;
    return new Promise((resolve) => {
      linkSelection.attr("x1", startPos.x).attr("y1", startPos.y).attr("x2", startPos.x).attr("y2", startPos.y).style("opacity", 0);
      const transition2 = linkSelection.transition().duration(finalConfig.duration).delay(finalConfig.delay || 0).ease(cubicInOut);
      this.activeAnimations.set(linkId, transition2);
      transition2.attr("x2", endPos.x).attr("y2", endPos.y).style("opacity", 1).on("end", () => {
   ...
   */
  createPulseAnimation(selection2, config = {}) {
    const finalConfig = { ...this.defaultConfig, ...config };
    const pulseId = `pulse-${Math.random()}`;
    const pulse = () => {
      const transition2 = selection2.transition().duration(finalConfig.duration).ease(sinInOut);
      this.activeAnimations.set(pulseId, transition2);
      transition2.attr("transform", "scale(1.1)").transition().duration(finalConfig.duration).attr("transform", "scale(1)").on("end", () => {
   ...
   */
  animateLayoutChange(nodeSelection, linkSelection, config = {}) {
    const finalConfig = { ...this.defaultConfig, ...config };
    return new Promise((resolve) => {
   ...
   */
  animateNodeEntrance(nodeSelection, config = {}) {
    const finalConfig = { ...this.defaultConfig, ...config };
    return new Promise((resolve) => {
   ...
   */
  animateNodeExit(nodeSelection, config = {}) {
    const finalConfig = { ...this.defaultConfig, ...config };
    return new Promise((resolve) => {
   ...
   */
  async runAnimationSequence(animations) {
...
var GraphRenderer = class {
   ...
   */
  initializeNodeManipulator(vault, parser) {
  ...
  renderGraph(nodes, links) {
    ...
    const node = g.append("g").attr("class", "nodes").selectAll("g").data(nodes).enter().append("g").attr("class", "node").attr("data-id", (d) => d.id).attr("data-number", (d) => d.zettel.number).call(this.createEnhancedDragBehavior(nodes));
  ...
  createEnhancedDragBehavior(nodes) {
    if (!this.nodeManipulator && this.currentGraph) {
